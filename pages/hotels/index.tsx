...
const Hotels: React.FC<HotelsProps> = ({ hotels }) => {
  const [price, setPrice] = useState<number>(3500);
  const [list, setList] = useState<HotelData[]>([]);
  const [checkedList, setCheckedList] = useState<string[]>([]);

  const handleCheckList = async () => {
    const { data } = await axios.get<{ hotels: HotelData[] }>("/api/facilities/search", {
      params: { val: checkedList },
    });
    if (data?.hotels) {
      setList(data.hotels);
    }
  ...
  const handlePrice = async () => {
    const { data } = await axios.get<{ hotels: HotelData[] }>("/api/facilities/range", {
      params: { price },
    });
    if (data?.hotels) {
      setList(data.hotels);
